...
// Using raw GitHub URLs for images
const LOGO_IMG = "https://raw.githubusercontent.com/Luyun1224/AI-questionnaire/main/logo.png";  
const CHIMEI_IMG = "https://raw.githubusercontent.com/Luyun1224/AI-questionnaire/main/Chimei.png";  
...
  // Mocking more structured feedback for robustness
  const feedbackHarvest = isITSession ? "學會了 Python 基礎與 API 串接，雖然有點難但很有用，希望能應用在數據分析上。" : "了解 AI 對行政效率的幫助，這對我寫公文很有幫助，尤其是摘要功能很實用。";
  const feedbackSuggestion = isITSession ? "程式碼部分太難了，跟不上，希望有講義和更慢的實作速度。" : "希望能多一點實作時間，時間有點趕，網路連線不太穩定。";
  const feedbackApplication = isITSession ? "嘗試導入日常業務流程，利用 AI 整理研究數據。" : "已經開始使用 AI 輔助郵件回覆和文件起草。";
...
    sat_scores,
    feedback: {
      harvest: feedbackHarvest,
      plan: feedbackApplication,
      suggestion: feedbackSuggestion,
      link: Math.random() > 0.8 && i % 4 === 0 ? "https://www.google.com/docs/about/" : ""
    }
  };
});
...
        {/* The original logic for colorClass was not fully utilized, defaulting to blue theme */}
      </div>
    </div>
...
           {/* Moved tooltip content below the axis */}
          </div>
        </foreignObject>
      )}
    </g>
...
      {children}
      {/* Ensure text wrapping is controlled by Tailwind classes: leading-relaxed, space-y-3 */}
    </div>
  </div>
);
...
      // Ensure post_scores and sat_scores are arrays of numbers
...
      // Robustly extract feedback, supporting both object and string formats
      if (row.feedback) {
...
            feedbackObj.harvest = row.feedback;
         }
      }
...
        id: index,
        role: row.role || '其他',
        // Calculate average scores for the four psychological constructs based on questionnaire grouping
...
  // Fetch Data with Fallback and Exponential Backoff
  useEffect(() => {
    const maxRetries = 3;
...
          // console.log(`Fetch failed. Retrying in ${delay}ms... (Attempt ${retryCount + 1})`); // Do not log retries
          setTimeout(() => attemptFetch(retryCount + 1), delay);
        } else {
          console.warn("API Fetch Failed after max retries, loading Mock Data:", err);
          setRawData(processSheetData(MOCK_DATA));
          setIsUsingMock(true);
          setLoading(false);
          setError("無法連接外部數據源，正在使用本地範例數據。");
        }
      }
    };
...
  // The original instructorGapData was removed as the analysis now uses the values directly for insights.
  // Retaining the core KPI logic for top cards
  const kpi = useMemo(() => {
...
  // Removed learningInsights as they were not used in the final structure
...
    if (filteredData.length === 0) return { chartData: [], highest: null, lowest: null };
    const dimScores = SATISFACTION_LABELS.map((label, idx) => {
        const total = filteredData.reduce((acc, cur) => {
            // Handle potential missing or undefined items gracefully
...
            // Dynamic color based on score (5-point scale)
            fill: avg >= 4.5 ? COLORS.primary : avg >= 4.0 ? COLORS.secondary : COLORS.accent
        };
    });
...
       
       // Calculate average satisfaction for core design items (first 4 items)
       const satDesign = roleData.reduce((acc, cur) => {
         const items = cur.satisfaction_items || [];
         return acc + (items.slice(0, 4).reduce((a, b) => a + b, 0) / 4);
       }, 0) / count;
       
       return {
         name: r,
...
  if (error && isUsingMock) return <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center text-[#D6604A]"><AlertCircle className="w-12 h-12 mb-4" /><h3 className="text-xl font-bold text-slate-800 mb-2">數據載入警示</h3><p className="mb-4 text-center max-w-md text-sm">{error}</p><p className="text-xs text-slate-500">（目前顯示本地範例數據）</p></div>;
  if (error) return <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center text-[#D6604A]"><AlertCircle className="w-12 h-12 mb-4" /><h3 className="text-xl font-bold text-slate-800 mb-2">數據載入錯誤</h3><p className="mb-4 text-center max-w-md text-sm">{error}</p></div>;
...
                      <PolarRadiusAxis angle={30} domain={[0, 5]} tick={false} axisLine={false} />
                      {Object.keys(COLORS.roles).map((role) => (<Radar key={role} name={role} dataKey={role} stroke={COLORS.roles[role]} strokeWidth={2} fill={COLORS.roles[role]} fillOpacity={0.1} />))}
